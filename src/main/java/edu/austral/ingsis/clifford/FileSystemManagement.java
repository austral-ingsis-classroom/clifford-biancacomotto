...
public final class FileSystemManagement{
    ...
    public Optional<String> mkdir(String name) {
        ...
        return Optional.of("'" + name + "' directory created"); //Optional.of(mensaje) SI TODO ESTA BIEN
    }
...
// ls pasar el current
        ...
        if (order.isPresent()) {
            switch (order.get()) {
                case "asc" -> children.sort((a, b) -> a.getName().compareTo(b.getName()));
                case "desc" -> children.sort((a, b) -> b.getName().compareTo(a.getName()));
                default -> { /* no hacemos nada */ }
            }
        } else {
            //  SI NO SE PASA orden explícito, ORDENAR ASCENDENTE igual sino mis test no pasan
            children.sort(Comparator.comparing(FileSystem::getName));
        }
        // OJO: Solo ordeno si order.isPresent()!
